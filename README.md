...
- Для админа: после `/start` доступны кнопки-разделы `Календарь`, `Записи`, `Услуги`, `Расписание`, `Настройки`. Кнопки, которым нужны данные, запускают пошаговый ввод, например добавить услугу, задать часы, создать месяц, записать клиента, удалить или перенести запись.
...
Командные форматы поддерживаются как fallback: `/schedule 2026-06`, `/service_add 30 Категория > Подкатегория > Название`, `/generate 2026-06 2`, `/appoint @username YYYY-MM-DD HH:MM`, `/book YYYY-MM-DD HH:MM`, `/move FROM_DATE FROM_TIME TO_DATE TO_TIME`.
