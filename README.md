...
- Для клиента: `/start` с помощью Qwen создает короткое приветствие на выбранном языке из профиля мастера и активного каталога, затем добавляет точный список услуг из базы и кнопки `Начать запись`, `Календарь` и `Мои записи`, без списка команд. Если модель недоступна, используется нейтральный шаблон без привязки к виду услуг. Месяц и неделя показываются картинками: свободные интервалы зеленые, занятые серые, а данные чужих записей в клиентскую версию не передаются. Во время любой интерактивной записи, включая кнопку `Записаться` у админа, после выбора услуг бот показывает приватную PNG-сетку на семь дней, затем кнопки доступных дат и, после выбора даты, кнопки конкретного времени. Клиент также может написать, произнести или прислать на фото: `хочу эпиляцию на полтора часа завтра вечером`. После выбора времени бот покажет детали и кнопки `Да`, `Нет`, `Найти другое`; запись создается только по `Да`. Команды `/calendar`, `/week`, `/services`, `/schedule 1 4`, `/book 3`, `/my`, `/move 1 3`, `/month 2026-08`, `/lang ru|en` остаются только как совместимый ручной режим.
- Для админа: после `/start` доступны кнопки-разделы `Календарь`, `Записи`, `Услуги`, `Расписание`, `Настройки`. Повседневная правка услуг идет через `Изменить услугу`: выбрать услугу, затем только нужное поле (`Цена / описание`, длительность, название или разделы) и отправить новое значение текстом либо голосом; остальные поля сохраняются. Фраза `измени только цену маникюра на 50 EUR` создает точечный предпросмотр и требует подтверждения. Отдельная кнопка `Заменить весь список` предназначена для редкой полной замены каталога; старые записи при этом сохраняются. Администратор также может написать или произнести: `запиши @client на эпиляцию завтра в 18:00 на полтора часа`, а также прислать такой запрос фотографией или скриншотом. Бот проверит свободное время и потребует подтверждение до создания записи; для занятого времени предложит до семи ближайших вариантов. Печатный текст на изображении обрабатывается локальным Tesseract; при низкой уверенности OCR, например для рукописного текста, используется текущая модель Qwen. В `Календарь` есть кнопки `Неделя` и `Свободное время`; обе отправляют PNG-сетку сразу на семь дней. Фразы `покажи график за неделю`, `покажи расписание на следующую неделю` и дата запускают тот же PNG-ежедневник. На нем видны свободные и закрытые интервалы, время записи, клиент и услуга. Кнопки, которым нужны данные, запускают пошаговый ввод, например добавить или удалить услугу, задать часы, создать или удалить месяц расписания, записать клиента, удалить или перенести запись.
